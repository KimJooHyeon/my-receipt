...
const PROMPT = `이 사진에 있는 모든 음식/물건/상품을 찾아 나열해주세요.

품목명(name) 규칙 — 가장 중요:
- 사진의 포장지/라벨/메뉴판에 **글자가 보이면 그 텍스트를 그대로** 사용
  - 예: "촉촉한 황치즈칩" → "촉촉한 황치즈칩" (O), "치즈" (X)
  - 예: "신라면 봉지면" → "신라면 봉지면" (O), "라면" (X)
  - 예: "스타벅스 아메리카노 톨" → "스타벅스 아메리카노 톨" (O), "커피" (X)
- 글자가 없으면 일반적인 한국어 이름 사용 (예: "사과", "감자")
- 절대 줄이거나 일반화하지 말 것
- 수량 정보는 name에 포함 X (수량은 quantity 필드에)
